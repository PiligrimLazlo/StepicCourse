package ru.pl.shoppinglist.data.contentprovider
...
import ru.pl.shoppinglist.data.ShopListMapper
...
class ShopListProvider : ContentProvider() {
    ...
    private val uriMatcher = UriMatcher(UriMatcher.NO_MATCH).apply {
        addURI("ru.pl.shoppinglist", "shop_items", GET_SHOP_ITEMS_QUERY)
        addURI("ru.pl.shoppinglist", "shop_items/#", GET_SHOP_ITEM_BY_ID_QUERY)
    }
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        when (uriMatcher.match(uri)) {
            GET_SHOP_ITEMS_QUERY -> {
                //Пример: "DELETE FROM shop_items WHERE id = ? AND name = ?"
                //selection: id = ? AND name = ?
                //selectionArgs: аргументы на месте знаков вопросов
